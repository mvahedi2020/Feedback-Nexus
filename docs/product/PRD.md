...
F01 and F02 remain separate source records after grouping. Their linked four-account union is a review aid only: it never replaces an individual account list, rewrites a source quote, or becomes a hidden group score. F01’s score remains its own `3 accounts × 4 impact × confidence ÷ 3 effort`; at 40% confidence, that is **1.6**, regardless of the four-account group context. A reviewer may record F01 as Planned with an empty rationale and an unreviewed simulated tag. Those fields invite explanation and review, but neither is an artificial gate or a delivery commitment.
