...
| Fixture recovery changes the reference sample | A session edit to a fallback fixture appears in a later reset or another test path | Use an isolated fixture copy for empty and rejected storage reads, and treat the seed as immutable reference data. |
| Ambiguous saved links alter evidence grouping | A group label contains padding or cannot be resolved to a saved record | Reject the saved payload and require explicit reset rather than guessing which evidence belongs together. |
