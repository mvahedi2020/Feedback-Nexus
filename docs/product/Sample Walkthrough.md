...
This is a public walkthrough of the fictional Northstar sample. It uses only browser-local data; no feedback is sent anywhere.
...
3. Compare that import-recovery evidence with **F04 — Save the weekly operations report**. F04 remains separate and starts as Inbox. This is a deliberate PM tradeoff: reach is a directional input, while the original quote, segment, confidence, and effort remain available to challenge the comparison.
...
5. Change one visible priority input. For example, F01 confidence can be **40%, 60%, 80%, or 100%**; its score updates from the displayed formula. The score is not a delivery commitment or an automatic recommendation.
