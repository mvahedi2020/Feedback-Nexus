...
const cloneSeed=()=>seed.map(item=>({...item,accounts:[...item.accounts]}));
export function readSavedFeedback(raw:string|null){if(!raw)return {items:cloneSeed(),warning:'',blocked:false};try{const parsed=JSON.parse(raw);if(!isFeedbackList(parsed))throw Error();return {items:parsed as Feedback[],warning:'',blocked:false};}catch{return {items:cloneSeed(),warning:'Saved data is unavailable or incompatible. Review or reset it before saving new feedback.',blocked:true};}}
export function useFeedback(){
const [initial]=useState(()=>{try{return readSavedFeedback(localStorage.getItem(key));}catch{return {items:cloneSeed(),warning:'Browser storage is unavailable. Changes work for this session only.',blocked:false};}});
...
function reset(){try{localStorage.removeItem(key);}catch{void 0;}setItems(cloneSeed());setBlocked(false);setWarning('');}
