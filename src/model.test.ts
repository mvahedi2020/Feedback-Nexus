...
import {seed,score,merge,groupAccounts,csv,newFeedbackId,normalizeEffort,uniqueAccounts,isFeedback,plannedFeedback} from './model';
...
it('exports planned feedback in the visible score order with provenance and explicit units',()=>{const result=csv([{...seed[0],status:'Planned',confidence:.4},{...seed[3],status:'Planned'}]);expect(result).toContain('"Segment","Source","Tag","Original record accounts","Impact","Confidence","Directional score"');expect(result).toContain('"Growth","Support interview example","Onboarding"');expect(result.indexOf('F04')).toBeLessThan(result.indexOf('F01'));});
it('derives a stable planned review set without changing source order',()=>{const result=plannedFeedback([{...seed[0],status:'Planned',confidence:.4},{...seed[3],status:'Planned'},seed[1]]);expect(result.map(item=>item.id)).toEqual(['F04','F01']);expect(seed[0].status).toBe('Inbox');});
